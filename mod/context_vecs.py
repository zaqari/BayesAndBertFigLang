...
def avgC(lexeme, context, vecs=vecs, layer_no=9):
    ...
    if len(candidates) < 1:
        candidates = [i for i in range(1, len(lexemes) - 1) if ((str(lexemes[i])[1:] in 'Ġ'+item) and (str(lexemes[i + 1])[1:] in 'Ġ'+item)) or ((str(lexemes[i])[1:] in 'Ġ'+item) and (str(lexemes[i - 1])[1:] in 'Ġ'+item))]
...
def nC(lexeme, context, vecs=vecs, layer_no=0):
    ...
    if len(candidates) < 1:
        candidates = [i for i in range(1, len(lexemes) - 1)
                      if ((str(lexemes[i])[1:] in 'Ġ'+item) and (str(lexemes[i + 1])[1:] in 'Ġ'+item))
                      or ((str(lexemes[i])[1:] in 'Ġ'+item) and (str(lexemes[i - 1])[1:] in 'Ġ'+item))]
